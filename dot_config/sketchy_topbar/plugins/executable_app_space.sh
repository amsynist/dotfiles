...
sketchy_topbar --set $NAME background.drawing=$SELECTED \
 ...
 do
   ...
   then
     ...
     do
       ...
       ICON=$($HOME/.config/sketchy_topbar/plugins/app_icon.sh "$APP" "$TITLE")
       LABEL+="$ICON"
       if [[ $j < $(($LENGTH-1)) ]]; then
         LABEL+=" "
       fi
   ...
   sketchy_topbar --set space.$sid label="$LABEL"
 done
fi
